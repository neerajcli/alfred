module.exports = {
    ...
    execute: async (client, message, args) => {
        ...
        let work = await db.get(`rob_${message.author.id}`);
        if (work !== null && timeout - (Date.now() - work) > 0) {
            ...
            let security = await db.get(`security_${user.id}`)
            if (security !== true) {
                ...
                if (chance <= 4) {
                    if (cash !== null) {
                        ...
                        await db.set('moneykk_' + message.author.id, money1)
                        let money = await db.get(`moneykk_${message.author.id}`)
                        await db.add(`cash_${message.author.id}`, money)
                        await db.sub(`cash_${user.id}`, money)
                    ...
                    if (ws === 0 && wb !== 0) {
                        ...
                        let loss = Math.round(Math.random() * ws)
                    } else {
                        let loss = Math.round(Math.random() * 1000)
                    }
                    if (loss > 5000) loss = 5000;
                    await db.set('loss1_' + message.author.id, loss)
                    const money11 = await db.get(`loss1_${message.author.id}`)
                ...
                await db.delete(`security_${user.id}`)
            }
