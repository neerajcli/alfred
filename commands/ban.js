...
module.exports = {
  ...
  execute: async (client, message, args) => {
     ...
     if (enable !== true) return message.channel.send('Mod logs channel should be set to use this command.')
      ...
      const voted = await db.get(`banr_${message.guild.id}${user.id}`)
    ...
    await db.set("moderator_" + message.guild.id + user.id, message.author.id)
    await db.set("banr_" + message.guild.id + user.id, true)
