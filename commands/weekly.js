module.exports = {
  ...
  execute: async (client, message, args) => {
    ...
    let weekly = await db.get(`weekly_${message.author.id}`);
    if (weekly !== null && timeout - (Date.now() - weekly) > 0) {
      ...
      await db.set('moneyweekly_' + message.author.id, money1)
      let money = await db.get(`moneyweekly_${message.author.id}`)
