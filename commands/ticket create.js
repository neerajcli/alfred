...
if(isPremium !== true) return message.channel.send('This command can only be used in premium servers.')
        ...
        let ticketNumber = await db.get('ticketnumber_' + message.guild.id)
        ...
        let pending = await db.get('ticketpending_' + message.author.id + message.guild.id)
        let pendingId = db.get('ticketchannelid_' + message.author.id + message.guild.id)
        ...
        let newTicketNumber = await db.get('ticketnumber_' + message.guild.id)
