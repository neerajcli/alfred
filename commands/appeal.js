            ...
            .then(async (msg) => {
                ...
                try {
                    ...
                    if (reaction.emoji.name === '1️⃣') {
                        ...
                        await db.set("appealtype_"+message.author.id,"u");
                        handleAppeal(userBlacklistQuestions, 'User Blacklist Appeal');
                    } else if (reaction.emoji.name === '2️⃣') {
                        const isSBlacklisted = await db.get("blguild_" + message.guild.id);
                        ...
                        await db.set("appealtype_" + message.guild.id, "s");
            ...
            const askQuestion = async () => {
                if (currentIndex < questions.length) {
                    ...
                    if(appealType=='User Blacklist Appeal') {
                        ...
                        await db.set('appealed_'+message.author.id,true);
                        await db.add('tappealed_'+message.author.id,1)
                    } else {
                        ...
                        await db.set('sappealed_'+message.guild.id,true);
                        await db.add('stappealed_'+message.guild.id,1)
                        await db.set('dmres_' + message.guild.id, message.author.id);
                    }
