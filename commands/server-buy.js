...
if(isPremium !== true) return message.channel.send('This command can only be used in premium servers.')
        ...
        const cash = await db.get(`cash_${message.author.id}`)
        let a = ["1", "2", "3", "4", "5"]
        if(!a.includes(args[0])) return message.channel.send('Invalid Item')
        let items = await db.get(`item${args[0]}_${message.guild.id}`)
        if(items === null) await db.set(`item${args[0]}_${message.guild.id}`, "Not-Set")
        let item = await db.get(`item${args[0]}_${message.guild.id}`)
        ...
        const price = await db.get(`price${args[0]}_${message.guild.id}`)
        if(cash < price) return message.channel.send('You dont have enough money in cash to buy this item!')
        await db.sub("cash_" + message.author.id, price)
        user.roles.add(role)
