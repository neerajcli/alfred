module.exports = {
    ...
    execute: async (client, message, args) => {
        ...
        await db.set('kkkkk_' + message.author.id, money1)
        await db.add('kkkkk_' + message.author.id, 2000)
        let money = await db.get(`kkkkk_${message.author.id}`)
        ...
        let security = await db.get(`security_${message.author.id}`)
        let coin = await db.get(`coin_${message.author.id}`)
        let big = await db.get(`big_${message.author.id}`)
        let cash = await db.get(`cash_${message.author.id}`)
        let userPremium = await db.get(`userpremium_${message.author.id}`)
        let serverPremium = await db.get(`serverpremium_${message.guild.id}`)
        ...
        if (item === "security") {
            ...
            await db.sub('cash_' + message.author.id, 3000)
            await db.set('security_' + message.author.id, true)
            ...
            await db.set('ggggg_' + message.author.id, chance1)
            let chance = await db.get(`ggggg_${message.author.id}`)
            if (chance <= 1) {
                message.channel.send(mysteriousEmbedRedeem)
                await db.add('redeem_' + message.author.id, 1)
            } else {
                message.channel.send(mysteriousEmbedMoney)
                await db.add('cash_' + message.author.id, money)
            }
            await db.sub('cash_' + message.author.id, 5000)
        } else if (item === "redeem") {
            ...
            await db.add('redeem_' + message.author.id, 1)
            await db.sub('cash_' + message.author.id, 20000)
            ...
            await db.set('coin_' + message.author.id, true)
            await db.sub('cash_' + message.author.id, 900000)
            ...
            await db.set('big_' + message.author.id, true)
            await db.sub('cash_' + message.author.id, 500000)
            ...
            await db.sub('cash_' + message.author.id, 1000000)
